...
%DIF DEL 01_draft.tex           Tue Oct 19 13:08:06 2021
%DIF ADD 02_draft_revised.tex   Thu Oct 21 18:09:50 2021
...
solution for inferring \DIFdelbegin \DIFdel{a }\DIFdelend \emph{plausible} \DIFdelbegin \DIFdel{metaweb }\DIFdelend \DIFaddbegin \DIFadd{interactions }\DIFaddend (despite the
...
the field of ecological network prediction, Strydom }\emph{\DIFadd{et al.}}
\DIFadd{(2021a) identified two challenges of interest to the prediction of
...
construct a }\emph{\DIFdel{probabilistic}} %DIFAUXCMD
\DIFdel{metaweb for a community for which we
have }\emph{\DIFdel{no}} %DIFAUXCMD
\DIFdel{prior trophic interaction data for the desired species
...
\DIFdel{2020b, a) and, based on phylogenetic relationships between
mammals globally (}\emph{\DIFdel{i.e.}}%DIFAUXCMD
\DIFdel{, phylogenetic tree Upham }\emph{\DIFdel{et al.}}
%DIFAUXCMD
\DIFdel{2019), infer a metaweb for the Canadian mammalian species pool
(interactions are treated as unknown in this instance).
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}
%DIFDELCMD < \hypertarget{fig:concept}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics{figures/figure-concept.png}
%DIFDELCMD < \caption{Overview of the phylogenetic transfer learning (and prediction)
%DIFDELCMD < of species interactions networks. Starting from an initial, known,
%DIFDELCMD < network, we learn its representation through a graph embedding step
%DIFDELCMD < (here, a truncated Singular Value Decomposition; Step 1), yielding a
%DIFDELCMD < series of latent traits (vulnerability traits representing species at
%DIFDELCMD < the lower trophic-level and generality traits representing species at
%DIFDELCMD < higher trophic-levels; \emph{sensu} Schoener (1989)); second, for the
%DIFDELCMD < destination species pool, we perform ancestral character estimation
%DIFDELCMD < using a phylogeny (here, using a Brownian model for the latent traits;
%DIFDELCMD < Step 2); we then sample from the reconstructed distribution of latent
%DIFDELCMD < traits (Step 3) to generate a probabilistic metaweb at the destination
%DIFDELCMD < (here, assuming a uniform distribution of traits), and threshold it to
%DIFDELCMD < yield the final list of interactions (Step 4).}\label{fig:concept}
%DIFDELCMD < }
...
Riva \& Stouffer 2016), strong evolutionary signature on prey choice
(Stouffer \emph{et al.} 2012), and strong phylogenetic signature in food
web intervality (Eklöf \& Stouffer 2016). Phylogenetic reconstruction
has also previously been used within the context of ecological networks,
namely understanding ancestral plant-insect interactions (Braga \emph{et
al.} 2021). Taken together, these considerations suggest that
phylogenies can reliably be used to transfer knowledge on species
interactions.
...
interactions for European mammals (Maiorano }\emph{\DIFadd{et al.}} \DIFadd{2020; known
interactions; }\textbf{\DIFadd{Maiorano2020TetEu?}}\DIFadd{) and, based on phylogenetic
relationships between mammals globally (}\emph{\DIFadd{i.e.}}\DIFadd{, phylogenetic tree
Upham }\emph{\DIFadd{et al.}} \DIFadd{2019), infer a metaweb for the Canadian mammalian
species pool (interactions are treated as unknown in this instance).
Following the definition of Dunne (2006), a metaweb is a network
analogue to the concept of a regional species pool. Specifically, a
metaweb is an inventory of all }\emph{\DIFadd{possible}} \DIFadd{interactions within
species likely to occurr within a spatially delimited area (the network
\(\gamma\)-diversity, in a sense). The metaweb is, therefore, }\emph{\DIFadd{not}}
\DIFadd{a prediction of the food web at any specific locale within the frontiers
of the spatial area it recovers, and will in fact have a different
structure (notably by having a larger connectance; see }\emph{\DIFadd{e.g.}}
\textbf{\DIFadd{Wood2015EffSpa?}}\DIFadd{). These local food webs are expected to be a
subset of both the species and the interactions of their metaweb, and
have been called ``metaweb realizations'' (Poisot }\emph{\DIFadd{et al.}} \DIFadd{2015).
The difference between a food web at a specific location and the metaweb
has to do with a variety of mechanisms, including species co-occurrence,
local environmental conditions, and local distribution of functional
traits. Nevertheless, the metaweb represents the total of functional,
phylogenetic, and macroecological processes (Morales-Castilla }\emph{\DIFadd{et
al.}} \DIFadd{2015), and therefore still holds valuable ecological information.
Because the metaweb can be down-sampled given appropriate knowledge of
local species composition (the network \(\alpha\)-diversity, in a
sense), it is possible to infer what may drive the structure of food
webs at finer spatial scales. This has been done for example for
tree-gallers-parasitoid systems (}\textbf{\DIFadd{Gravel2018BriElt?}}\DIFadd{), fish
trophic interactions (Albouy }\emph{\DIFadd{et al.}} \DIFadd{2019), tetrapods trophic
interactions (O'Connor }\emph{\DIFadd{et al.}} \DIFadd{2020), and crop-pests networks
(Grünig }\emph{\DIFadd{et al.}} \DIFadd{2020). Whereas the original metaweb definition was
based on presence/absence, we focus on }\emph{\DIFadd{probabilistic}} \DIFadd{metawebs;
...
of success of a Bernoulli trial (see }\emph{\DIFadd{e.g.}} \DIFadd{Poisot }\emph{\DIFadd{et al.}}
\DIFadd{2016), which allows properly weigh interactions as a function of how
likely they are.
}
...
phylogenetic structure (Stock 2021).

\hypertarget{data-used-for-the-case-study}{%
\section{Data used for the case
study}\label{data-used-for-the-case-study}}

We use data from the European metaweb assembled by
\DIFdelbegin \DIFdel{Maiorano }\emph{\DIFdel{et
al.}} %DIFAUXCMD
\DIFdelend (\DIFdelbegin \DIFdel{2020b), following the definition of the metaweb first introduced
by Dunne (2006), }\emph{\DIFdel{i.e.}} %DIFAUXCMD
\DIFdel{an inventory of all possible interactions
...
expected to have a subset of both the species and the interactions of their metaweb (Poisot }\emph{\DIFdel{et al.}} %DIFAUXCMD
\DIFdel{2012). This being said, as the metaweb represents the total of functional, phylogenetic, and
macroecological processes (Morales-Castilla }\emph{\DIFdel{et al.}} %DIFAUXCMD
\DIFdel{2015), it is
thus still worthy of ecological attention. We deduced }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Maiorano2020TetEu?}}\DIFadd{).
}

\textbf{\DIFadd{TANYA TODO}} \DIFadd{description of the data
}
...
applied by \DIFdelbegin \DIFdel{Maiorano }\emph{\DIFdel{et al.}} %DIFAUXCMD
\DIFdel{(2020b}\DIFdelend \DIFaddbegin \DIFadd{(}\textbf{\DIFadd{Maiorano2020TetEu?}}\DIFaddend ) in the European metaweb). The
...
}\emph{\DIFadd{i.e.}} \DIFadd{an algorithm that can be understood mathematically, but
...
(Halevy }\emph{\DIFadd{et al.}} \DIFadd{2009); in fact, in an instance of ecological
forecasting of spatio-temporal systems, model-free approaches
(}\emph{\DIFadd{i.e.}} \DIFadd{drawing all of their information from the data)
outperformed model-informed ones (Perretti }\emph{\DIFadd{et al.}} \DIFadd{2013).
}
...
taxonomic cleanup steps are done using \texttt{GBIF.jl}
(\DIFdelbegin \DIFdel{Dansereau \&
Poisot 2021}\DIFdelend \DIFaddbegin \textbf{\DIFadd{Dansereau2021SimJl?}}\DIFaddend ). The network embedding and analysis is
done using \texttt{EcologicalNetworks.jl} (Poisot \emph{et al.} 2019;
Banville \emph{et al.} 2021). The phylogenetic simulations are done
using \texttt{PhyloNetworks.jl} (Solís-Lemus \emph{et al.} 2017) and
...
network edges\DIFdelbegin \DIFdel{(}\emph{\DIFdel{i.e.}} %DIFAUXCMD
\DIFdel{interactions; }\DIFdelend \DIFaddbegin \DIFadd{. Interstingly, the latent variables do not need to be
prouced by decomposing the network itself; in a recent contribution,
}\DIFaddend Runghen \emph{et al.} \DIFaddbegin \DIFadd{(}\DIFaddend 2021) \DIFaddbegin \DIFadd{show that deep artificial neural networks
are able to reconstruct the left and right subspaces of a RDPG, in order
to predict human movement networks from individual/location metadata.
This is an exciting opportunity, as it opens up the possibility of using
additional predictors}\DIFaddend .
...
\DIFdelbegin \DIFdel{Maiorano }\emph{\DIFdel{et al.}} %DIFAUXCMD
\DIFdel{(2020b}\DIFdelend \DIFaddbegin \DIFadd{(}\textbf{\DIFadd{Maiorano2020TetEu?}}\DIFaddend ) and O'Connor \emph{et al.} (2020), this
...
algorithm would return point values for all simulations, one could in theory either estimate the parameters of a
distribution for each tip, or draw randomly from the outputs. In all
cases, }\DIFdelend \DIFaddbegin \DIFadd{. The choice of the uniform distribution was made because the
algorithm returns a minimum and maximum point estimate for the value,
and given this information, }\DIFaddend the \DIFaddbegin \DIFadd{uniform distribution is the one with
...
\DIFaddbegin \DIFadd{Dallas }\emph{\DIFadd{et al.}} \DIFadd{(2017) suggest that most links in ecological
networks may be cryptic, }\emph{\DIFadd{i.e.}} \DIFadd{uncommon or otherwise hard to
...
}\emph{\DIFadd{et al.}} \DIFadd{2002). In the light of these observations, the results in
...
(Hoffmann }\emph{\DIFadd{et al.}} \DIFadd{2019): in this perspective, high quality
...
\DIFaddend Cirtwill \emph{et al.} (2019) previously made the point that network
...
except for the legacy of settler colonialism\DIFaddbegin \DIFadd{; indeed, the use of
ecological data is not an apolitical act (Nost \& Goldstein 2021).
Operating under the framework of national boundaries, and relying on
data infrastructures designed to answer questions within them}\DIFaddend , \DIFdelbegin \DIFdel{and operating under them
}\DIFdelend must be
done under the clear realization that \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{this logic }\DIFaddend contributed to the
...
Arsov, N. \& Mirceva, G. (2019). \DIFdelbegin \DIFdel{Network Embedding: An Overview. }\emph{\DIFdel{arXiv:1911.11726 }%DIFDELCMD < {[}%%%
\DIFdel{cs, stat}%DIFDELCMD < {]}%%%
}%DIFAUXCMD
\DIFdelend \DIFaddbegin \emph{\DIFadd{Network Embedding: An Overview}}\DIFadd{.
...
Bersier, L.-F., \emph{et al.} (2006). \DIFdelbegin \DIFdel{ConsumerResource }\DIFdelend \DIFaddbegin \DIFadd{Consumer--Resource }\DIFaddend Body-Size
...
Graph Embedding: Problems, Techniques and Applications. }\emph{\DIFdel{arXiv
preprint arXiv:1709.07604}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \emph{\DIFadd{A Comprehensive
Survey of Graph Embedding: Problems, Techniques and Applications}}\DIFadd{.
...
Cirtwill, A.R., \DIFdelbegin \DIFdel{Eklf}\DIFdelend \DIFaddbegin \DIFadd{Eklӧf}\DIFaddend , A., Roslin, T., Wootton, K. \& Gravel, D. (2019).
...
stabilize them. }\emph{\DIFadd{Trends in Biochemical Sciences}}\DIFadd{, 29, 331--334.
}
...
\leavevmode\DIFdelbegin %DIFDELCMD < \hypertarget{ref-Dansereau2021SimJl}{}%%%
\DIFdelend \DIFaddbegin \hypertarget{ref-Dallas2017PreCry}{}\DIFaddend %
\DIFdelbegin \DIFdel{Dansereau, G. \& Poisot, T. (2021). SimpleSDMLayers.jl and GBIF.jl: A
Framework for Species Distribution Modeling in Julia. }\DIFdelend \DIFaddbegin \DIFadd{Dallas, T., Park, A.W. \& Drake, J.M. (2017). Predicting cryptic links
in host-parasite networks. }\DIFaddend \emph{\DIFdelbegin \DIFdel{Journal of
Open Source Software}\DIFdelend \DIFaddbegin \DIFadd{PLOS Computational Biology}\DIFaddend }, \DIFdelbegin \DIFdel{6,
2872.
...
and suitability overlap under climate change. }\emph{\DIFadd{Communications
Biology}}\DIFadd{, 3, 1--10.
}

\leavevmode\hypertarget{ref-Halevy2009UnrEff}{}%DIF > 
\DIFadd{Halevy, A., Norvig, P. \& Pereira, F. (2009). The Unreasonable
Effectiveness of Data. }\emph{\DIFadd{IEEE Intelligent Systems}}\DIFadd{, 24, 8--12.
}
...
Rubinstein, S.M., }\emph{\DIFadd{et al.}} \DIFadd{(2019). Machine learning in a
data-limited regime: Augmenting experiments with synthetic data uncovers
order in crumpled sheets. }\emph{\DIFadd{Science Advances}}\DIFadd{, 5, eaau6792.
}

\leavevmode\hypertarget{ref-Holm2019DefBla}{}%DIF > 
\DIFadd{Holm, E.A. (2019). In defense of the black box. }\emph{\DIFadd{Science}}\DIFadd{, 364,
26--27.
}
...
Thuiller, W. (\DIFdelbegin \DIFdel{2020a}\DIFdelend \DIFaddbegin \DIFadd{2020}\DIFaddend ). Data from: Tetra-EU 1.0: A species-level trophic
meta-web of European tetrapods.

\leavevmode\DIFdelbegin %DIFDELCMD < \hypertarget{ref-Maiorano2020TetEu}{}%%%
%DIF < 
\DIFdel{Maiorano, L., Montemaggiori, A., Ficetola, G.F., O'Connor, L. \&
Thuiller, W. (2020b). TETRA-EU 1.0: A species-level trophic metaweb of
European tetrapods. }\emph{\DIFdel{Global Ecology and Biogeography}}%DIFAUXCMD
\DIFdel{, 29,
1452--1457.
}%DIFDELCMD < 

%DIFDELCMD < \leavevmode%%%
\DIFdelend \hypertarget{ref-Marco2018ChaHum}{}%
...
in Real Food Webs: Weak Links in Long Loops. }\emph{\DIFadd{Science}}\DIFadd{, 296,
1120--1123.
}
...
}\emph{\DIFadd{Environment and Planning E: Nature and Space}}\DIFadd{, 25148486211043503.
}

\leavevmode\DIFaddend \hypertarget{ref-OConnor2020UnvFoo}{}%
O'Connor, L.M.J., Pollock, L.J., Braga, J., Ficetola, G.F., Maiorano,
L., \DIFdelbegin \DIFdel{Martinez-Almoyna}\DIFdelend \DIFaddbegin \DIFadd{Martinez‐Almoyna}\DIFaddend , C., \emph{et al.} (2020). Unveiling the food webs
...
experimental data. }\emph{\DIFadd{Proceedings of the National Academy of
Sciences}}\DIFadd{, 110, 5253--5257.
}
...
The dissimilarity of species interaction networks. }\emph{\DIFdel{Ecology
Letters}}%DIFAUXCMD
\DIFdel{, 15, 1353--1361.
}%DIFDELCMD < 
...
linear filtering and singular value decomposition. }\emph{\DIFdel{arXiv:2105.14973 }%DIFDELCMD < {[}%%%
\DIFdel{q-bio}%DIFDELCMD < {]}%%%
}%DIFAUXCMD
\DIFdelend \DIFaddbegin \emph{\DIFadd{Imputing the mammalian virome
with linear filtering and singular value decomposition}}\DIFadd{. Available at:
...
}\emph{\DIFadd{Oikos}}\DIFadd{, 124, 243--251.
}
...
}\emph{\DIFdel{arXiv:1404.6520 }%DIFDELCMD < {[}%%%
\DIFdel{q-bio}%DIFDELCMD < {]}%%%
}%DIFAUXCMD
\DIFdelend \DIFaddbegin \emph{\DIFadd{How to partition diversity}}\DIFadd{.
...
Desjardins-Proulx, P., \emph{et al.} (2021a). \DIFdelbegin \emph{\DIFdel{A Roadmap Toward
...
predicting species interaction networks (across space and time).
}\emph{\DIFadd{Philosophical Transactions of the Royal Society B: Biological
Sciences}}\DIFadd{, 376, 20210063.
}\DIFaddend 
