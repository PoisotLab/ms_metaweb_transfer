...
%DIF DEL reviews/01_ELE/01_draft.tex           Tue Oct 19 13:08:06 2021
%DIF ADD reviews/01_ELE/01_draft_revised.tex   Thu Oct 28 19:43:56 2021
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:scree}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics{figures/figure-screeplot.png}
%DIFDELCMD < \caption{Left: representation of the screeplot of the singular values
%DIFDELCMD < from the t-SVD on the European metaweb. The screeplot shows no obvious
%DIFDELCMD < drop in the singular values that may be leveraged to automatically
%DIFDELCMD < detect a minimal dimension for embedding, after \emph{e.g.} Zhu \&
%DIFDELCMD < Ghodsi (2006). Right: cumulative fraction of variance explained by each
%DIFDELCMD < dimension up to the rank of the European metaweb. The grey lines
%DIFDELCMD < represent cutoffs at 50, 60\ldots{} 90\% of variance explained. For the
%DIFDELCMD < rest of the analysis, we reverted to an arbitrary threshold of 60\% of
%DIFDELCMD < variance explained, which represented a good tradeoff between accuracy
%DIFDELCMD < and reduced number of features.}\label{fig:scree}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:subspaces}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics{figures/figure-subspaces.png}
%DIFDELCMD < \caption{Visual representation of the left (green/purple) and right
%DIFDELCMD < (green/brown) subspaces, alongside the adjacency matrix of the food web
%DIFDELCMD < they encode (greyscale). The European metaweb is on the left, and the
%DIFDELCMD < imputed Canadian metaweb (before data inflation) on the right. This
%DIFDELCMD < figure illustrates how much structure the left sub-space captures. As we
%DIFDELCMD < show in fig.~\ref{fig:degree}, the species with a value of 0 in the left
%DIFDELCMD < subspace are species without any prey.}\label{fig:subspaces}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:inflation}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics{figures/figure-validation.png}
%DIFDELCMD < \caption{Left, comparison of the probabilities of interactions assigned
%DIFDELCMD < by the model to all interactions (grey curve), the subset of
%DIFDELCMD < interactions found in GLOBI (red), and in the Strong \& Leroux (2014)
%DIFDELCMD < Newfoundland dataset (blue). The model recovers more interaction with a
%DIFDELCMD < low probability compared to data mining, which can suggest that
%DIFDELCMD < collected datasets are biased towards more common or easy to identify
%DIFDELCMD < interactions. Right, distribution of the in-degree and out-degree of the
%DIFDELCMD < mammals from Canada in the reconstructed metaweb. This figure describes
%DIFDELCMD < a flat, relatively short food web, in which there are few predators but
%DIFDELCMD < a large number of preys.}\label{fig:inflation}
%DIFDELCMD < }
%DIFDELCMD < %%%
