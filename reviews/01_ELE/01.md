...
> submission, we have emphasized the fact that our case study is extrernally
...
> We will list the main changes we mad related to each main point below, and
> have made far more extensive responses to each point in the actual reviews.
...
> we just published (a mew months ago) a comprehensive review of network
> inference, in which we identify challenges that motivated this manuscript. We
> now mention this in the discussion, and point readers that want to read a
> detailed survey of the methods for network prediction (including the difficult
> question of validation of unstructured data) to the review.
...
> metaweb on two independant datasets (GLOBI: 366 interactions, 33 missed, 91%
...
> shows, we do not think we are over-estimating by a large amount. In fact, the
> marginal figure (from Supp. Mat. 1) shows that RDPG can almost always remove
> false positive interactions that were artifically introduced in the data. This
> being said, we have clarified that the metaweb implies that all interactions,
> even the very rare ones, should be considered, and that this is especially
> true in a probabilistic metaweb (introduction; **TK**). We revisit this
> question in the discussion (**TK**).
...
> Strong & Leroux and GLOBI data (see at **TK** and **TK**), which is exactly
> similar to what the reviewer suggests. We reach a very good predictive
