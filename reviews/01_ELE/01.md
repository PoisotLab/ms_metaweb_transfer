...
> will see from the responses, we took each and every comment made by the three
> reviewers to heart, and have made many additions to the text. Our
> understanding of the comments is that the manuscript was clear to read, and
> that the underlying scholarship was robust; most of the comments were asking
> for clarifications, or for additional arguments that show the method to be
> actually robust. This has guided our effort in revising, and we are hopeful
> that the manuscript in its revised form meets the standard of Ecology Letters.   
>   
...
> We have added three appendices to (i) show that the method is not prone to
> overfitting, (ii) add simulations using a Normal (rather than Uniform)
> distribution, which results is severely over-predicting the number of
> interactions, and (iii) compare the functional differences between random
> subgraphs induced from the Canadian and European metaweb, to show that the
> differences between locations are not "averaged out", as one referee
> rightfully asked. We are confident that these additional results considerably
> strengthen both the generality or our approach, and its application to
> real-life situations.
>   
> If we may be allowed a single tongue in cheek comment, the non-insignificant
> portion of Canadian co-authors would like to emphasize that we are, indeed,
> "real-life", and although this may not have came across in the original
> submission, we have emphasized the fact that our case study is externally
> validated on the very limited set of data regarding the diet of Canadian
> mammals we were able to find.
...
> We appreciate the frank assessment by the associate editor, and the
> distillation of the reviewers concerns in four core points; we are confident
> that the thoroughness of our revision, and the results now presented in
> supplementary material, will establish that an updated version (albeit not a
> radically different one) is likely to meet the standards of Ecology Letters.
> We list below the main changes we mad related to each main point, and have
> made far more extensive responses to each point in the actual reviews.
...
> inference Strydom 2021 et al., in which we identify challenges that motivated
> this manuscript. We now mention this in the discussion, and point readers that
> want to read a detailed survey of the methods for network prediction
> (including the difficult question of validation of unstructured data) to the
> review.
...
> maximal entropy distribution given only bounds of an interval), and made a few
> manipulations to extract a normal distribution from the same data; as
> presented in Supp. Mat. 2, the normal distribution model does what reviewer 1
> was afraid of, *i.e.* over-predicting interactions. Specifically, the figure
> in the margins shows how the normal distribution model assigns strong
> probabilities to interactions that should be rare according to the uniform
> model; we disccus in Supp. Mat. 2 why this leads to overestimating some
> network properties. It notably predicts a lot of trophic interactions between
> basal species, which are ecologically unrealistic. We have therefore kept the
> uniform distribution for the manuscript.
...
> As was already the case in the original submission, but we did make this
> clearer in the **TK**, we externally validate the reconstructed Canadian
...
> metaweb (*i.e.*, how it can be operationalized, see **TK**), and get back to
> this point in the discussion in the context of synthetic data inflation of
> empirical datasets (**TK**).
...
> generators in the discussion, specifically **TK**. As an aside, our method is
> no more of black box than a PCA; indeed, as we now mention in the text at
> **TK**, PCA is a special case of SVD, and so ecologists should be familiar
> with the concepts.
...
> recently reviewed by Strydom 2021 et al., now cited in the introduction at
> **TK**; this review identified a number of challenges for the field to solve
> in order to move forward, which we discuss at **TK**, thereby outlining how
> our approach can be useful. We are confident that this will provide readers
> with a stronger justification for why our approach is solving problems that
> are not currently solvable.
...
> We have clarified the Runghen 20201 et al. citation at **TK** --- their aim
> was to predict human movement from external metadata. We hope that this
> clarifies how different our method is. We understand that the literature is in
> part self-referential; this is because this manuscript is part of a larger
> research program; we are pleased to note that the reviewer has not identified
> citations that were not justifiable, or citations that were missing,
> suggesting that we are crediting ideas appropriately.
...
> in the text at **TK**, but have not discussed the normal model further. We
> also show in Supp. Mat. 1 that RDPG is extremely good at eliminating spurious
> interactions, making us extremely confident that we are not over-predicting
> interactions.
...
> especially true in a probabilistic metaweb (introduction; **TK**). We revisit
> this question in the discussion (**TK**).
...
> clarified this distinction when introducing the concept of a metaweb at
> **TK**. We also provide some elements of answers to the legitimate question of
> "Why then do we need a metaweb?", specifically at **TK** and **TK**.
> Furthermore, drawing on a recent review of network prediction challenges, we
> discuss at **TK** the fact that having a metaweb can be used to facilitate the
> prediction of local food webs; this last point, indeed, remains a formidable
> challenge that our method is not intended to solve, but we discuss how it can
> be used at a more local scale at **TK**.
...
> (see the discussion at **TK** showing the existence of various food web
...
> the case for examples now listed at **TK**. To check that different species
> pools would not result in networks with the same structure, we provide a
> simple simulation as Supp. Mat. 3 (cited at **TK**), wherein we sub-sample the
> Canadian and European species list, and show that there exists a distribution
> of network structure in a multi-variate space, both within and across regions.
> This result is reproduced in the figure in the margin.
...
> The information was present in the original manuscript, specifically at
> **TK**. Quoting from the manuscript: "After taxonomic cleaning and
...
> respectively at **TK** and **TK**, and the demonstration of changes in Supp.
> Mat. 3, will have alleviated this concern.
...
> Strong & Leroux (2014) and GLOBI data (see at **TK** and **TK**), which is
...
> it to add to the abstract and introduction, notably at **TK**.
...
> in the methods. Most importantly, we now emphasize at **TK** that SVD is a
> general case of PCA, a method with which ecologists are very likely familiar.
> We hope that this will help develop a better instinct for how our method
> operates.
...
> $\mathbf{L}\mathbf{U}$. We have added this information at **TK**. In
...
> we clarified at **TK** following comments from reviewer 1, the "traits" here
> are more accurately described at latent variables, but their similarity is
> indeed a predictor of the expected similarity of interactions.
...
> This is correct; we have clarified this sentence at **TK**.
...
> are. We are therefore confident that the risk of overfitting is minimal. An
> example of the results from Supp. Mat. 1, specifically showing that RDPG
> maintains a ROC-AUC even with extreme data perturbations, is presented in the
> margin.
...
> the method. That being said, our discussion of metawebs now includes
> non-trophic examples (**TK**). We have cited the references suggested by the
> reviewer in the introduction (**TK**).