...
**TK** *big fancy overall words about stuff done*
...
> in (~l 83 .md) "infer a metaweb for the Canadian mammalian species pool (using
...
> **@here** Respectfully push back on this one. As this is a methods manuscript
> it seems negligent to 'remove' any text detailing the methodology from the
> main text since if we want people to be able to repeat this for a different
> setting we want to provide them with the best tool set possible. Arguably
> there are sections that could be removed from the introduction and discussion
> however some of these were included as a response to the first round of review
> and thus are probably crucial for helping to contextualize the presented
...
> paragraph, the *Data used for the case study* section has also been reduced
> (**note for Tim - the 'fluff' still in there was included as a response to the
> last round of reviews**), and as per reviewer 2's recommendation one of the
> paragraphs discussing thresholding has been combined and collapsed with the
> other.
...
> An excellent question and something we have addressed in-text as well (~l. 123
> in .md) In short, we don't necessarily need to use one super tree and (*e.g.*
...
> **@here** probably expanding in the legend
